...
echo 'Domain to mask the Phishing URL with (http or https), ex: https://fuckindia.com, http://fuckisrael.org, https://domain.com, https://google.com :'
echo -en "\e[32m=>\e[0m "
read mask
url_checker $mask
echo -e '\nType social engineering words:(like free-money, best-tricks)'
echo -e "\e[31mDon't use space just use '-' between social engineering words\e[0m"
echo -en "\e[32m=>\e[0m "
read words
if [[ -z "$words" ]]; then
    echo -e "\e[31m[!] No words.\e[0m"
    echo -e "\nGenerating KREPTONS PHISH Link...\n"
    final=$mask@$shorter
    echo -e "Here is the KREPTONS PHISH URL:\e[32m ${final} \e[0m\n"
    exit
fi
if [[ "$words" =~ " " ]]; then
